...
int ntt_e[N];
IL void polyksm(int *B, int *A, int degA, int b) {
    polyln(ntt_e, A, degA);
    for(int i=0;i<=degA;i++) ntt_e[i] = (1ll * b * ntt_e[i]) % mod;
    polyexp(B, ntt_e, degA);
}

template<typename Tp> IL void readm(Tp& x) {
    x=0; int f=1; char ch=getchar();
    while(!isdigit(ch)) {if(ch=='-') f=-1; ch=getchar();}
    while(isdigit(ch)) {x = (x * 10ll + ch-'0') % mod; ch=getchar();}
    x *= f;
}
